...
describe('wall (Mur — defensive building)', () => {
  ...
  it('appears right before the M9 market in the stable BUILDING_IDS order (was last until M9)', () => {
    ...
    // the stable after the market, M13 appended the watchtower, M15 the forge and M16 the paladin
    // last; the wall now sits sixth-to-last, still after the academy. Pins the append-only order through M16.
    ...
    expect(BUILDING_IDS[BUILDING_IDS.length - 5]).toBe('market')
    expect(BUILDING_IDS[BUILDING_IDS.length - 6]).toBe('wall')
  })
